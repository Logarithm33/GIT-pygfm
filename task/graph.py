"""Graph classification: SFT, fine-tune, and eval functions."""
...
def eval_graph(model, data, split, params):
    ...
    if setting in ['base', 'base_zero_shot']:
        ...
        train_value, val_value, test_value = eval_graph_few_shot(
            model, data, split, params,
        )

    else:
        raise ValueError(f"Invalid setting: {setting}")

    return {"train": train_value, "val": val_value, "test": test_value,
...
def eval_graph_few_shot(model, data, split, params):
    ...
    test_idx_s = torch.tensor(
        [idx for task in split['test']['support'] for idx in task.values()]
    ).reshape(-1)
    test_idx_q = torch.tensor(
        [idx for task in split['test']['query'] for idx in task.values()]
    ).reshape(-1)
    test_label_s = torch.tensor(
        [idx for task in split['test']['support_label'] for idx in task.values()]
    ).reshape(params['n_task'], -1)
    test_label_q = torch.tensor(
        [idx for task in split['test']['query_label'] for idx in task.values()]
    ).reshape(params['n_task'], -1)
    test_idx = np.concatenate([test_idx_s, test_idx_q])

    test_loader = DataLoader(
        data[test_idx], batch_size=params['bs'], num_workers=1, shuffle=False,
    )
    ...
    z_s = z[:len(test_idx_s)].reshape(
        params['n_task'], data.num_classes * 2 * params['n_shot'], -1,
    )
    z_q = z[len(test_idx_s):].reshape(
        params['n_task'], data.num_classes * 2 * params['n_query'], -1,
    )
    ...
    if not val_as_test:
        val_idx_s = torch.tensor(
            [idx for task in split['val']['support'] for idx in task.values()]
        ).reshape(-1)
        val_idx_q = torch.tensor(
            [idx for task in split['val']['query'] for idx in task.values()]
        ).reshape(-1)
        val_label_s = torch.tensor(
            [idx for task in split['val']['support_label'] for idx in task.values()]
        ).reshape(params['n_task'], -1)
        val_label_q = torch.tensor(
            [idx for task in split['val']['query_label'] for idx in task.values()]
        ).reshape(params['n_task'], -1)
        val_idx = np.concatenate([val_idx_s, val_idx_q])

        val_loader = DataLoader(
            data[val_idx], batch_size=params['bs'], num_workers=1, shuffle=False,
        )
        z_list = []
        ...
        z_s = z[:len(val_idx_s)].reshape(
            params['n_task'], data.num_classes * 2 * params['n_shot'], -1,
        )
        z_q = z[len(val_idx_s):].reshape(
            params['n_task'], data.num_classes * 2 * params['n_query'], -1,
        )
