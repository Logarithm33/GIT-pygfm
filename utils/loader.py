"""Data loader constructors for pretraining, SFT, and finetuning."""

from torch_geometric.loader import NeighborLoader, LinkNeighborLoader, DataLoader
from utils.utils import mask2idx


def clean_data(data):
    """Remove list-type attributes from PyG Data (causes issues with collate)."""
    for k in data.keys():
...
def get_pt_loader(data, train_nodes, params):
    """Pretraining loader: samples subgraphs around task nodes."""
    return NeighborLoader(
        data,
        input_nodes=train_nodes,
        num_neighbors=[params['fanout']] * params["num_layers"],
...
def get_sft_loader(data, params):
    """SFT loader: depends on task type."""
    task = params["task"]
...
def get_ft_loader(data, split, params):
    """Finetune loader: depends on task and setting."""
    task = params["task"]
    ...
    if task == "node":
        ...
        if setting in ['base', 'base_zero_shot']:
            ...
            val_loader = DataLoader(
                val_dataset, batch_size=10000, shuffle=False, num_workers=1,
            )
            test_loader = DataLoader(
                test_dataset, batch_size=10000, shuffle=False, num_workers=1,
            )
        elif setting in ['few_shot', 'zero_shot', 'in_context']:
            val_loader = DataLoader(
                data, batch_size=params["bs"], shuffle=True, num_workers=1,
            )
            test_loader = val_loader
        else:
            raise ValueError('The setting is not supported.')
