"""Train/val/test split logic for base, few-shot, zero-shot, and in-context settings."""
...
def get_split(data, params):
    ...
    if setting in ['base', 'base_zero_shot']:
        ...
        if task == 'graph':
            return few_shot_split_graph(data, params)
        return few_shot_split(data, params)
    else:
...
def base_split(data, params):
    ...
    if task == 'node':
        has_mask = (hasattr(data, 'train_mask') & hasattr(data, 'val_mask')
                    & hasattr(data, 'test_mask'))
        has_masks = (hasattr(data, 'train_masks') & hasattr(data, 'val_masks')
                     & hasattr(data, 'test_masks'))
        if has_masks:
        ...
        num_graphs = len(data)
        idx = torch.load(
            osp.join(params['data_path'], params['dataset'], 'processed', 'data.pt')
        )[0]
...
def few_shot_split(data, params):
    ...
    for split in splits:
        if params['no_split']:
            train_idx = val_idx = test_idx = mask2idx(
                split['train'] + split['val'] + split['test']
            )
        else:
            train_idx = mask2idx(split['train'])
            val_idx = mask2idx(split['val'])
            test_idx = mask2idx(split['test'])
        ...
        if not no_training:
            ...
            for idx in ways_idx:
                ...
                train_mask_fs.extend(
                    np.random.choice(way_idx_train, params['n_train'], replace=False)
                )
        else:
            print("Does not need to select samples for fine-tuning.")
        train_mask_fs = idx2mask(train_mask_fs, num_samples)

        for _ in range(params['n_task']):
            ...
            for idx in way_idx:
                ...
                tmp_idx_s.extend(
                    np.random.choice(way_idx_s, params['n_shot'], replace=False)
                )
                tmp_idx_q.extend(
                    np.random.choice(way_idx_q, params['n_query'], replace=False)
                )
...
# ── Few-shot split (graph) ─────────────────────────────────────────

def few_shot_split_graph(data, params):
    splits = base_split(data, params)
    labels = data.y.view(-1, data.num_classes)
    ...
    for split in splits:
        ...
        if not no_training:
            train_idx_fs, train_label_fs = get_instances_each_task(
                labels, train_mask, params['n_train'],
            )
            train_idx_fs = np.concatenate(list(train_idx_fs.values()))
        else:
            print("Does not need to select samples for fine-tuning.")

        for _ in range(params['n_task']):
            cur_idx_s, cur_labels_s = get_instances_each_task(
                labels, train_mask, params['n_shot'],
            )
            cur_idx_q, cur_labels_q = get_instances_each_task(
                labels, test_mask, params['n_query'],
            )
        ...
        if val_as_test:
            ...
            for _ in range(params['n_task']):
                ...
                cur_idx_q, cur_labels_q = get_instances_each_task(
                    labels, val_mask, params['n_query'],
                )
        ...
        fs_splits.append({
            "train": train_idx_fs,
            "val": {
                "support": val_idx_s, "query": val_idx_q,
                "support_label": val_label_s, "query_label": val_label_q,
            },
            "test": {
                "support": test_idx_s, "query": test_idx_q,
                "support_label": test_label_s, "query_label": test_label_q,
            },
        })
...
def get_instances_each_task(labels, mask, instance_per_task=10):
    ...
    for task, (p_idx, n_idx) in enumerate(zip(idx_pos, idx_neg)):
        if len(p_idx) == 0:
            p_idx = np.array([0] * instance_per_task)
        if len(n_idx) == 0:
            n_idx = np.array([0] * instance_per_task)

        p_idx = np.random.choice(
            p_idx, instance_per_task,
            replace=False if len(p_idx) > instance_per_task else True,
        )
        n_idx = np.random.choice(
            n_idx, instance_per_task,
            replace=False if len(n_idx) > instance_per_task else True,
        )

        cur_idx = np.concatenate((np.array([], dtype=int), p_idx, n_idx))
        cur_labels = np.concatenate((
            np.zeros(len(p_idx)) + task,
            tasks * np.ones(len(n_idx)) + task,
        ))
