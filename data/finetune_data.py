...
def get_data(params):
    """Dispatch to the appropriate data loader based on dataset name and task."""
    dataset_name = params['dataset']
    task = params['task']

    if dataset_name in citation_datasets + ecommerce_datasets:
        if task in ('node', 'link_pred'):
            return single_graph(params)
        else:
            raise ValueError(f"Task {task} not found for {dataset_name}")
...
# ── Citation / E-commerce single-graph loader ───────────────────────
...
def split_timestamps(ts, num_splits):
    ...
    for i in range(ts.min(), ts.max(), (ts.max() // num_splits) + 1):
        ...
        if i == ts.max() // num_splits * (num_splits - 1):
            ...
            mx = i + ts.max() // num_splits
        splits.append((int(mn), int(mx)))
    return splits
...
def get_snapshot(data, start, end, idx, to_undirected=False):
    ...
    return Data(
        node_text_feat=data.x, edge_index=edge_index, edge_text_feat=edge_attr,
...
def temporal_graph(params):
    ...
    data = Data(
        x=e_feat, edge_index=torch.stack([src, dst], dim=0), edge_attr=r_feat[rel_idx],
